...
**Чтобы не писать «продолжай» каждый раз:** **«Продолжай по NEXT_PLAN»** или **«Выполняй NEXT_PLAN»** — агент берёт следующий шаг из [NEXT_PLAN.md](NEXT_PLAN.md), выполняет его (тесты, правки, агенты при необходимости) и переходит к следующему без остановки на вопросе «продолжать?».
