...
- **NotebookLM: экспорт 21 источника:** Созданы [документация/NOTEBOOKLM_EXPORT_SOURCES.md](../документация/NOTEBOOKLM_EXPORT_SOURCES.md) (процедура экспорта через MCP) и [документация/notebooklm_sources/00_INDEX.md](../документация/notebooklm_sources/00_INDEX.md) (индекс). MCP вернул «Authentication expired» — нужен `nlm login`, затем повторить запрос на выгрузку.
...
- **Продолжай / B.1 базовая линия:** повторный прогон CartPole 50 эп. (сид 42, steps=10, night=200) — медиана 9.0, mean 9.4 (совпадает с базовой линией). В HOW_TO_CONTINUE добавлен блок «При „продолжай“ сейчас»: Q.1, B.1 (референс genesis-agi или B.1.опц), опции D.3/sifs_agents.
